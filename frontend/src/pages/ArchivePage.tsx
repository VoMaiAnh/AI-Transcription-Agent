...
export function ArchivePage() {
  ...
  const items = useMemo<ArchiveItem[]>(() => {
    ...
    return [...transcriptionItems, ...ttsItems]
      ...
      .filter((item) => {
        const haystack = `${item.name} ${item.model} ${item.detail}`.toLowerCase();
  ...
  const handleDelete = async (item: ArchiveItem) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete archive item');
    }
  };

  const handleSubtitleDownload = async (item: ArchiveItem, format: 'srt' | 'vtt') => {
    try {
      const { content, filename, mediaType } = await api.downloadSubtitle(item.id, format);
    ...
    <div className="archive-page">
      <section className="page-heading archive-heading">
        <div>
          ...
          <p>Access and manage historical transcription and text-to-speech projects.</p>
        </div>
      ...
      <section className="glass-card archive-filters">
        <div className="segmented-control">
          {(['all', 'transcription', 'tts'] as ArchiveType[]).map((item) => (
            ...
            >
              {item === 'all' ? 'All Files' : item === 'tts' ? 'TTS' : 'Transcriptions'}
            </button>
          ))}
        </div>
        <label className="archive-search">
          Search Archive
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filename, model, text..." />
        </label>
        <button className="studio-button ghost" type="button" onClick={loadArchive}>
          Refresh
        </button>
      </section>
      ...
      <section className="glass-card archive-table-card">
        {loading ? (
          ...
          <div className="empty-panel">No archive items match the current filters.</div>
        ) : (
            ...
            {items.map((item) => (
              <div className="archive-row" key={`${item.type}-${item.id}`}>
                <div className="archive-name">
                  <span className="media-thumb small">{item.type === 'tts' ? 'TT' : 'AV'}</span>
                  <div>
                ...
                <div className="archive-actions">
                  {item.type === 'transcription' && (
                    <Link className="archive-link-button" to={`/editor/${item.id}`}>
                      Open
                    </Link>
                  )}
                  {item.hasSubtitles && (
                    <>
                      <button type="button" onClick={() => handleSubtitleDownload(item, 'srt')}>SRT</button>
                      <button type="button" onClick={() => handleSubtitleDownload(item, 'vtt')}>VTT</button>
                    </>
                  )}
                  <button className="danger" type="button" onClick={() => handleDelete(item)}>
                    Delete
                  </button>
                </div>
              </div>
